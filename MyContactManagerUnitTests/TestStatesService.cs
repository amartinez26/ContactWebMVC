...
namespace MyContactManagerUnitTests
{
    public class TestStatesService
    {
        ...
        {
            CreateMocks();
            //the state service will leverage the mock and not need to go against a real db
        ...
        {
           return new State() { Id = 14, Name = "Illinois", Abbreviation = "IL" };
        }
        [Theory]
        ...
        public async void TestGetAllStates(string name, string abbr, int index)
        {
            //will grab the states we set up locally to this file so you should get the 20 above not all 50 in our repo.
            ...
            states[index].Abbreviation.ShouldBe(abbr, StringCompareShould.IgnoreCase);
        }

        [Fact]
        public async Task TestGetAState()
        {
            //integer doesnt matter here it will always return the one we put above (Illinois)
            var state = await _statesService.GetAsync(1);
            state.ShouldNotBeNull();
            state.Name.ShouldBe("Illinois", StringCompareShould.IgnoreCase);
            state.Abbreviation.ShouldBe("IL", StringCompareShould.IgnoreCase);
        }
    }
}